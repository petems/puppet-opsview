...
Puppet::Type.type(:opsview_servicegroup).provide :opsview, :parent => Puppet::Provider::Opsview do
  @req_type = 'servicegroup'
  ...
  def flush
    if @servicegroup_json
      @updated_json = @servicegroup_json.dup
    else
  ...
  def initialize(*args)
    ...
    # Save the JSON for the servicegroup if it's present in the arguments
    if args[0].class == Hash and args[0].has_key?(:full_json)
      @servicegroup_json = args[0][:full_json]
    end
    ...
    @servicegroup_properties = @property_hash.dup
  end

  # Return the current state of the servicegroup in Opsview.
  def servicegroup_properties
    @servicegroup_properties.dup
  end
